//! Module containing the multi price auction algorithm.
use crate::{Auction, Bid, Bids, Sale, Sales};

/// Resolves bids into sales using the multi price algorithm.
///
/// # Arguments
...
///
pub fn multi_price(auction: &Auction, mut bids: Bids) -> Sales {
    ...
    winning_bids
        .iter()
        .map(|bid| Sale::new(bid.id, bid.amount, bid.quantity))
        .collect()
}
...
mod test {
    ...
    #[test]
    fn multi_price_returns_empty_for_no_bids() {
        let bids: Bids = vec![];
    ...
    #[test]
    fn multi_price_returns_all_bids_with_large_lot() {
    ...
    #[test]
    fn multi_price_return_some_of_bids_with_small_lot() {
        let bids: Bids = vec![bid![10, 1], bid![20, 1]];
        let auction = AuctionBuilder::new()
            .strategy(AuctionStrategy::MultiPrice)
            .lots(1)
    ...
    #[test]
    fn multi_price_all_sales_have_different_amount() {
        let bids: Bids = vec![bid![10, 1], bid![20, 1]];
        let auction = AuctionBuilder::new()
            .strategy(AuctionStrategy::MultiPrice)
            .lots(2)
    ...
    #[test]
    fn multi_price_partially_fulfilled() {
    ...
    #[test]
    fn multi_price_reserve_price_applied() {
